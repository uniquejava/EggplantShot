...
→ Remaining annotate tools (marker), magnifier — see [`docs/selection-refine.md`](docs/selection-refine.md) (shared refine rules + per-tool notes; split to `annotate-*.md` only when a tool section grows). Shape + arrow + pencil + mosaic + text + step + OCR + undo/redo + snip history (`,` / `.`, disk) + `AnnotationPayload` are in.
